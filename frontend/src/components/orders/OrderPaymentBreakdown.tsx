...
export function OrderPaymentBreakdown({ itemsSubtotal, packagingAmount, packagingRate, shippingAmount, total, calculatedItemsSubtotal, paymentMethod }: OrderPaymentBreakdownProps) {
  const subtotal = itemsSubtotal ?? calculatedItemsSubtotal ?? total;
  const hasExactBreakdown = shippingAmount !== undefined || packagingAmount !== undefined;
