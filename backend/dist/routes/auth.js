...
router.post('/register', async (req, res) => {
    try {
        ...
        try {
            ...
            const hint = raw.includes('SMTP not configured')
...
router.post('/login', async (req, res) => {
    try {
        ...
        const rows = await (0, db_1.dbQuery)('SELECT * FROM users WHERE LOWER(email) = ? LIMIT 1', [normalizedEmail]);
        const row = rows[0];
        if (!row || !isBcryptHash(row.password) || !(await bcryptjs_1.default.compare(String(password), row.password))) {
...
router.post('/forgot-password/request', async (req, res) => {
    try {
        ...
        const rows = await (0, db_1.dbQuery)('SELECT id, email, status, role FROM users WHERE LOWER(email) = ? LIMIT 1', [email]);
        const row = rows[0];
        // Always respond with success to avoid leaking account existence.
        if (!row)
            return res.json({ message: 'If the email exists, we sent a code.' });
        if (row.status && row.status !== 'active')
            return res.json({ message: 'If the email exists, we sent a code.' });
        // Basic throttle: 1 OTP per 60s per user.
        const recent = await (0, db_1.dbQuery)('SELECT id FROM password_reset_otps WHERE user_id = ? AND created_at > (NOW() - INTERVAL 60 SECOND) LIMIT 1', [row.id]);
        if (recent.length)
            return res.json({ message: 'If the email exists, we sent a code.' });
        const { otp, expires } = (() => {
        ...
        await (0, db_1.dbExecute)('INSERT INTO password_reset_otps (user_id, otp_hash, expires_at, consumed) VALUES (?, ?, ?, 0)', [row.id, hashOtp(otp), expires]);
        try {
            await (0, email_1.sendPasswordResetOtp)(row.email, { otp, minutes: RESET_OTP_MINUTES });
        }
        catch {
...
router.post('/forgot-password/verify', async (req, res) => {
    try {
        ...
        if (!(0, db_1.isDbConnected)())
            return res.status(503).json({ message: 'Database unavailable' });
        await ensurePasswordResetTable();
        const rows = await (0, db_1.dbQuery)('SELECT id, name, email, role, status FROM users WHERE LOWER(email) = ? LIMIT 1', [email]);
        const userRow = rows[0];
        if (!userRow)
            return res.status(400).json({ message: 'Invalid code' });
        if (userRow.status && userRow.status !== 'active')
            return res.status(400).json({ message: 'Invalid code' });
        const candidates = await (0, db_1.dbQuery)('SELECT id, otp_hash, expires_at FROM password_reset_otps WHERE user_id = ? AND consumed = 0 ORDER BY created_at DESC LIMIT 5', [userRow.id]);
        ...
        await (0, db_1.dbExecute)('UPDATE password_reset_otps SET consumed = 1 WHERE id = ?', [match.id]);
