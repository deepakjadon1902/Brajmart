    ...
    if ((0, db_1.isDbConnected)()) {
        try {
            const [categoryRows, productRows, blogRows] = await Promise.all([
                (0, db_1.dbQuery)('SELECT name, product_count FROM categories WHERE archived_at IS NULL AND name IS NOT NULL AND name <> "" ORDER BY (display_order IS NULL OR display_order = 0) ASC, display_order ASC, created_at DESC LIMIT 40'),
                (0, db_1.dbQuery)('SELECT name, slug, price, in_stock, category, updated_at FROM products WHERE archived_at IS NULL AND slug IS NOT NULL AND slug <> "" ORDER BY updated_at DESC LIMIT 40'),
                (0, db_1.dbQuery)("SELECT title, slug, excerpt FROM blogs WHERE archived_at IS NULL AND slug IS NOT NULL AND slug <> '' AND status = 'published' ORDER BY COALESCE(published_at, created_at) DESC LIMIT 20").catch(() => []),
            ]);
            for (const row of categoryRows || []) {
                const slug = slugify(row.name);
                if (slug)
                    categories.push(`- ${String(row.name).trim()}${Number(row.product_count || 0) > 0 ? ` (${Number(row.product_count)} products)` : ''}: ${SITE_URL}/category/${slug}`);
            }
            ...
            for (const row of blogRows || []) {
                const slug = slugify(row.slug);
                if (slug)
                    blogs.push(`- ${String(row.title || '').trim()}: ${SITE_URL}/blog/${slug}`);
            }
        }
        catch (err) {
